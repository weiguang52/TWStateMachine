...
class OfflineNPYLoader:
    ...
    def _load_npy_data(self):
        ...
        if not os.path.exists(self.npy_file):
            self.logger.error(f'[离线加载器] 文件不存在: {self.npy_file}')
            self.npy_data = None
            self.total_frames = 0
            return

        try:
            # 和 ik_redirection_new.py 的 load_data 保持一致
            data = np.load(open(self.npy_file, "rb"), allow_pickle=True)

            # 约定：data 是 (T, 22, 3) 或 (T, 29, 3) 这样的 motion 序列
            self.npy_data = data
            self.total_frames = len(self.npy_data)

            self.logger.info(
                f'[离线加载器] 成功加载: {os.path.basename(self.npy_file)}\n'
    ...
    def load_next_batch(self, frames_per_batch):
        ...
        if self.npy_data is None:
            self.logger.error('[离线加载器] NPY数据未加载')
            return None
        ...
        if end_frame > self.total_frames:
            ...
            part2_frames = end_frame - self.total_frames
            part2 = self.npy_data[:part2_frames]
...
class MainNode(Node):
    def __init__(self):
        ...
        # self.NPZ_FILE = "/home/sunrise/TWStateMachine/src/robot_state_machine/robot_state_machine/utils/IK_Redirection/input/motion_4.npy"
